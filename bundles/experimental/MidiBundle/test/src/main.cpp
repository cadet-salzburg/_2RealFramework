...
void newMidiInputData(void *userdata, Data &data)
{
	m.lock();
...
void newMidiOutputData(void *userdata, Data &data)
{
	std::cout<<" Midi Output data"<<std::endl;
	m.lock();
	std::vector< unsigned char > const& vec = data.getData< std::vector< unsigned char > >();
	for ( unsigned int i=0; i<vec.size(); ++i )
	{
...
int main(int argc, char *argv[])
{
	...
	try 
	{
		...
		BlockIdentifier midiInput = tutorialSystem.createBlock(tutPlugin, "MidiInputService", fpsTrigger);
		tutorialSystem.setup(midiInput);
		tutorialSystem.registerToNewData( midiInput, "inputmidi", newMidiInputData );
		BlockIdentifier midiOutput = tutorialSystem.createBlock(tutPlugin, "MidiOutputService", fpsTrigger);
		tutorialSystem.setup(midiOutput);

		tutorialSystem.registerToNewData( midiOutput, "outputmidi", newMidiOutputData );
	}
