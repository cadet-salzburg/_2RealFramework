...
namespace _2Real
{
	...
	typedef std::multimap< long, EngineData, std::greater< long > > DataMap;
	...
	public:
		...
		const unsigned int size() const { return m_DataMap.size(); }
		void insert(TimestampedData const& data) { m_DataMap.insert(data); }
		DataBuffer::iterator erase(DataMap::iterator it) { return m_DataMap.erase(it); }
		void clear() { m_DataMap.clear(); }
		void popOldest() { if (!this->empty()) { DataMap::iterator it = m_DataMap.end(); it--; this->erase(it); } }
		const bool empty() const { return m_DataMap.empty(); }
		DataBuffer::iterator end() { return m_DataMap.end(); }
		DataBuffer::iterator begin() { return m_DataMap.begin(); }
		DataBuffer::const_iterator begin() const { return m_DataMap.begin(); }
	...
	{
		return (m_DataMap.size() >= m_MaxSize);
	}
	...
	public:
		...
		Inlet(ParameterData const& metadata, BufferPolicy &policy, const unsigned int bufferSize);
		~Inlet();
