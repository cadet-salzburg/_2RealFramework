...
namespace _2Real
{
	...
	RunnableStateError::RunnableStateError() :
		AbstractRunnableState(1, "this runnable is ready to be updated and waiting for a thread to do so")
	{
	}

	void RunnableStateError::setUp(Runnable &runnable) const
	{
	}
	...
	const bool RunnableStateError::shutDown(Runnable &runnable) const
	{
		runnable.shutdown();
		return true;
	}
