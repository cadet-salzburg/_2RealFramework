...
namespace _2Real
{
	...
	RunnableStateReady::RunnableStateReady() :
		AbstractRunnableState(2, "this runnable is ready to be updated and waiting for a thread to do so")
	{
	}

	void RunnableStateReady::setUp(Runnable &runnable) const
	{
		//hmmmm?
	}
	...
	const bool RunnableStateReady::tryBeginUpdate() const
	{
		return true;
	}
	...
	const bool RunnableStateReady::shutDown(Runnable &runnable) const
	{
		runnable.shutdown();
		return true;
	}
